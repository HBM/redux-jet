...
export const fetch = (connection, expression, id, onStatesDidChange) => {
  return ensurePeer(connection)
    .then((peer) => {
      ...
      // ensure asArray option is set for sorting expressions.
      // this way the onStatesDidChange callback always gets the complete
      // sorted array passed in as data (non-differential).
      if (expression.sort) {
        expression.sort.asArray = true
      }
      fetcher = new Fetcher()
